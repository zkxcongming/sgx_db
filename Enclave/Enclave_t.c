...
typedef struct ms_ecall_initdb_t {
	const char* ms_sql;
	size_t ms_sql_len;
} ms_ecall_initdb_t;
...
static sgx_status_t SGX_CDECL sgx_ecall_execute_sql(void* pms)
{
	...
	if (_tmp_sql != NULL && _len_sql != 0) {
		...
		if (memcpy_s((void*)_in_sql, _len_sql, _tmp_sql, _len_sql)) {
	...
	ms->ms_retval = ecall_execute_sql((const char*)_in_sql);
err:
...
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_initdb_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_initdb_t* ms = SGX_CAST(ms_ecall_initdb_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const char* _tmp_sql = ms->ms_sql;
	size_t _len_sql = ms->ms_sql_len ;
	char* _in_sql = NULL;
	...
	ecall_initdb((const char*)_in_sql);
err:
	if (_in_sql) free((void*)_in_sql);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_ecall_opendb, 0},
		{(void*)(uintptr_t)sgx_ecall_execute_sql, 0},
		{(void*)(uintptr_t)sgx_ecall_closedb, 0},
		{(void*)(uintptr_t)sgx_ecall_initdb, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[25][4];
} g_dyn_entry_table = {
	25,
	{
		{0, 0, 0, 0, },
