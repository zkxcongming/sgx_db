...
sgx_status_t ecall_execute_sql(sgx_enclave_id_t eid, int* retval, const char* sql)
{
	sgx_status_t status;
...
sgx_status_t ecall_initdb(sgx_enclave_id_t eid, const char* sql)
{
	sgx_status_t status;
	ms_ecall_initdb_t ms;
	ms.ms_sql = sql;
	ms.ms_sql_len = sql ? strlen(sql) + 1 : 0;
